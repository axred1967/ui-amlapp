app2.controller('risk_profile04_4d', function ($scope,$http,$state,$translate,$timeout,$stateParams,$interval) {
  //gestisco lo state parameter
    ...
    $scope.back=function(passo){
      if (passo>0){
        $scope.pages['risk_final_4d']={action:'',location:$scope.page.location,prev_page:$state.current.name,agg:$scope.page.agg}
        localstorage('pages', JSON.stringify($scope.pages));
        $state.go('risk_final_4d' ,{pages:$scope.pages})
        return;
      }
      if (passo==-1){
...
//		$scope.main.AddPage="add_contract"
  ...
  $scope.main.viewName="Aspetti Connessi alla Operazione 2"
