...
function save_kyc(type)
{
   ...
   var company_lisence_image = $('#company_lisence_image').val();
    ...
    else if(!letterNumber(comany_fiscal_id)) swal("",validcomany_fiscal_idmsg)
    ...
    else
    {
        ...
        $.ajax ({
            type: "POST",
            url: SERVICEURL2,
            data: {"action":"add_company",agency_id:agency_id,company_name:company_name,company_address:company_address,comany_fiscal_id:comany_fiscal_id,company_authorised_by:company_authorised_by,company_authorisation_date:company_authorisation_date,fiscal_budget:fiscal_budget,company_lisence_image:company_lisence_image,compnay_doc_image :compnay_doc_image,customer_id_act:customer_id_act },
            crossDomain: true,
            success:function(responceData){
                window.localStorage.removeItem("actforcompany");
                    $('#loader_img').hide();
                    $('#kyc_button1').show();
                    $('#kyc_button2').show();
