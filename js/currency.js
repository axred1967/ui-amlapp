(function() {
    ...
    MaterialCurrencyTextfield.prototype.Constant_ = {
        Pattern: new RegExp(leftPat + virgul +rightPat),
        CommaPattern: new RegExp(leftCp,"g"),
        FormattedPattern: new RegExp(leftFp+decSep+ centerFp+ virgul + rightFp)
...
*/
    ...
    MaterialCurrencyTextfield.prototype.onFocus_ = function (event) {
		...
		this.input_.value = this.input_.value.replace(this.virgul, "-");
        while (this.input_.value.indexOf(this.decSep) !== -1) {
            this.input_.value = this.input_.value.replace(this.decSep, "");
        }
		this.input_.value = this.input_.value.replace("-",this.virgul);
    ...
    MaterialCurrencyTextfield.prototype.onBlur_ = function (event) {
       ...
       this.input_.value = this.input_.value.replace("-", this.virgul);
        if (!this.element_.classList.contains(this.CssClasses_.IS_INVALID) && this.element_.classList.contains(this.CssClasses_.IS_DIRTY)) {
            ...
            if (typeof matches[2] === "undefined" || matches[2] === this.decSep || matches[2] === "") {
                matches[2] = "0";
            }
            ...
            //Insert commas
            this.input_.value = this.input_.value.toString().replace(this.Constant_.CommaPattern, this.decSep);
        }
